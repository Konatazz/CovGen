...
class model_trainer():
    ...
    def __init__(self, diff_model, batchSize, numSteps, epochs, lr, device, Lambda, saveDir, numSaveSteps, use_importance, p_uncond=None, max_world_size=None, load_into_mem=False, optimFile=None):
        ...
        if dev != "cpu":
            ...
            # self.model = DDP(diff_model.cuda(), device_ids=[0], find_unused_parameters=False)
        else:
    ...
    def compute_coverage(self, activations):
        total_neurons = 0
        activated_neurons = 0

        for activation in activations:
            num_neurons = activation.numel()
            total_neurons += num_neurons
            activated_neurons += (activation > 0).sum().item()

        coverage_ratio = activated_neurons / total_neurons if total_neurons > 0 else 0
    ...
    def lossFunct(self, epsilon, epsilon_pred, v, x_0, x_t, t):
        ...
        coverage_ratio = self.compute_coverage(original_model.activations)
        target_coverage = 1
        coverage_loss = (coverage_ratio - target_coverage) ** 2
        # # Get the combined loss
        # loss_comb = loss_simple + loss_vlb
        loss_comb = loss_simple + loss_vlb + coverage_loss
    ...
    def train(self, data_path, num_data, cls_min, reshapeType):
        ...
        self.model.train()
        selected_classes = list(range(101))
        dataset = CustomDataset(data_path, num_data, cls_min, selected_classes, scale=reshapeType, loadMem=self.load_into_mem)
        # print(f"**************************************************")
        # print(f"{dataset.num_data} data loaded in")
        # print(f"**************************************************")

        dataset.get_first_n_classes_images(n_classes=5, n_images_per_class=10, output_dir='C:/Users/Administrator/Desktop/diffusion_change_unet/output_images/TRAIN')
        ...
        for epoch in range(self.model.module.defaults["epoch"], self.epochs+1):
            ...
            for step, data in enumerate(data_loader):
                ...
                if num_steps%self.numSteps == 0:
                    ...
                    if is_main_process():
                        print(f"epoch #{epoch} step #{num_steps}   Latest loss estimate: {round(losses_comb_s.cpu().detach().item(), 6)}")
